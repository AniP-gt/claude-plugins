...
上記 5 カテゴリ以外の観点（ロック設計・並行性・データ破壊リスク・パストラバーサル等）は、呼び出し側の追加指示があれば対象に含める。ただし以下のルールを厳守する。

- **実害シナリオが具体的に提示できないものは報告しない**。「理論上の競合可能性」「想定外のパス名」のような机上のリスクで埋めない
- **既存コードと同等のリスク（新規導入差分でないもの）は LOW 以下にする**。今回の差分でリスクが増えていないなら、報告対象は「観察」レベルにとどめ、対応推奨度は「別タスク起票」とする
- **重大度は実害深刻度で判定する**。攻撃面・運用面いずれかから現実的な悪用・事故シナリオを示せない場合は LOW を超えない
...
| # | カテゴリ | 重大度 | ファイル:行 | 内容 | 実害シナリオ | 対応推奨度 |
|---|---------|--------|------------|------|------------|-----------|
| 1 | npx未ピン留め | HIGH | scripts/foo.py:79 | @sentry/mcp-server@latest | 上流パッケージ乗っ取り時にビルドで任意コード実行 | やる |
...
各列の運用:
- **実害シナリオ**: 攻撃者・運用事故いずれかから具体的に悪用・破壊される経路を 1 文で記述する。「理論上競合し得る」のような机上表現は禁止
- **対応推奨度**: `やる` / `やらない` / `別タスク起票` のいずれかを記す。判断根拠（既存同等リスク・スコープ外・実害なし等）も簡潔に併記する

重大度の基準:
- **HIGH**: npx @latest / SQLインジェクション / 認証情報ハードコード / 認証バイパス / 任意ファイル書き込み
- **MEDIUM**: シェル変数不整合 / 予測可能な/tmpパスに機密情報 / 権限昇格余地
- **LOW**: npxバージョン未指定（@latestなし） / ログファイルの固定パス / 既存コードと同等の既知リスク
