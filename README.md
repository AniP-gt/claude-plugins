...
Jina AI Remote MCP経由でWeb検索・URLからのmarkdown抽出・arXiv/SSRN論文検索・リランク・分類などを実行するプラグイン。`mcp.jina.ai/v1` にStreamable HTTPで接続する。

環境変数 `JINA_API_KEY` の設定が必要です。API キーは <https://jina.ai/api-dashboard/key-manager/> から取得できます。MCPツールは deferred tools として on-demand ロードされる。
