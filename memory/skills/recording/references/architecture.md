...
両者ともマウント前提（staging なし）。保存後に **`scripts/wiki/enqueue.py --kind <kind>` で wiki キューへ追加 → `scripts/wiki/wiki-runner.sh` を fire-and-forget 起動** する。Codex が kind 別に統合先（`wiki/references.md` / `wiki/decisions.md`）を更新し、cocoindex も変更検知で自動再インデックスする。
