...
- `${CLAUDE_PLUGIN_ROOT}/scripts/recording/main_memory.py` — memory 専用 cocoindex エントリポイント（memory プラグイン自身の venv（`memory/scripts/.venv`）で実行）
...
- `${CLAUDE_PLUGIN_ROOT}/scripts/setup_db.sh` — memory 専用 PostgreSQL database の冪等セットアップ
...
memory プラグインは **専用 venv（`memory/scripts/.venv`）と専用 database（`memory`）** を持ち、cocoindex プラグインから完全に独立している。`uv` がインストールされていない環境では cocoindex 更新は自動的にスキップされる。
...
memory プラグインの `scripts/` は `uv` の独立 venv（`scripts/.venv`）と独立 database（`memory`）を持つため、cocoindex プラグインのバージョン変更や再インストールに対して頑健。
