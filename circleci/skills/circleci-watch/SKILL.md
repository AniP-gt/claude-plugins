...
1. **入力解釈**: ユーザー指示から URL / slug+branch / workflow-id を抽出。曖昧な場合は CircleCI MCP の最新ワークフロー取得ツールで確定する
...
5. **失敗時**: CircleCI MCP の失敗ログ取得ツールで失敗ジョブのログを補足取得し、ユーザーへ提示
...
- 単発の調査は CircleCI MCP のツール群（failures / tests / status / flaky / rerun / config 等）を直接呼び出す。目的状態到達まで待つ場合のみ本 skill を使う
