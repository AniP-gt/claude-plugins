"""compass プラグイン: ベクトル検索 + voyage rerank でコードのエントリーポイントを発見する
...
（compassindex_<host>_<project>__chunks）。
...
DB URL は COMPASS_DATABASE_URL（既定 postgres://postgres:postgres@localhost:15432/compass）。
"""
import argparse
import os
import pathlib
...
# Secrets fallback chain（main.py と同じ順序）
...
# compass 用 cocoindex.toml から rerank/embedding 設定を流し込む
...
if _COMPASS_CONFIG.exists():
    ...
    try:
        ...
        print(f"[compass/search] warn: failed to parse {_COMPASS_CONFIG}: {_e}", file=sys.stderr)
...
def get_table_name(project_dir: str) -> str:
    ...
    return f"compassindex_{sanitized}__chunks".lower()
