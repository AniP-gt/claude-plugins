...
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1]))  # plugin root を import path に追加（lib/ を解決）
...
#    テンプレ参照先は plugin root 直下の templates/cocoindex.toml.example（recording/ の親）。
...
# episodic 専用 database への接続 URL。
# setup_db.sh が ~/.config/episodic/.env を雛形（scripts/templates/episodic.env.example）から
# 自動生成し、その中に EPISODIC_DATABASE_URL が定義される。本ファイル冒頭の load_dotenv で
# 環境変数へ展開済みのはずなので、ここでは未設定を fail-fast で弾く（認証情報のソース直書きを避ける）。
DATABASE_URL = os.environ.get("EPISODIC_DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError(
        "EPISODIC_DATABASE_URL が未設定です。"
        " `scripts/setup_db.sh` を実行して ~/.config/episodic/.env を生成するか、"
