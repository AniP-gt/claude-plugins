...
# session 経路と挙動を揃え、外出時でも記事を保存できるようにする。
...
$(MEMREC_SCRIPTS_DIR="${PLUGIN_ROOT_FOR_LIB}/scripts" python3 - <<'PY' 2>/dev/null
import os, sys
...
if [[ -z "$OUT_PATH" ]]; then
    ...
    if [[ $IS_STAGED -eq 1 ]]; then
        # SMB 不在: staging 配下に kind サブディレクトリ付きで退避（sync-pending が後で移送）
        OUT_DIR="$FALLBACK_DIR/web/$DATE_DIR"
