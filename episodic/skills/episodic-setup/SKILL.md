...
| `uv` | episodic プラグイン専用 venv（`episodic/.venv`）の管理と `cocoindex update` 実行 | `brew install uv` または `curl -LsSf https://astral.sh/uv/install.sh \| sh` |
...
`/cocoindex-setup` で PostgreSQL コンテナ起動・`~/.config/cocoindex/secrets.env` 初期化を済ませる。episodic プラグインは独立した venv（`episodic/.venv`）と独立した database（`memory`）を持つため、cocoindex プラグインのコード変更による影響は受けない。
...
コピー元は plugin source ツリー直下の `bin/` / `lib/` / `session/` / `recording/` / `wiki/` / `pyproject.toml` / `uv.lock` / `templates/cocoindex.toml.example`。Codex hook の command は `~/.config/episodic/codex-hook-runtime/bin/session-start.sh` を指す。
