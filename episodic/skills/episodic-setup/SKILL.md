...
| `EPISODIC_DATABASE_URL` | episodic 専用 PostgreSQL 接続 URL（既定 `postgres://postgres:postgres@localhost:15432/episodic`） |
...
うまくいかない場合は `/tmp/episodic/session-{hook,runner,sync,retry}.log` を確認。
...
```bash
rm -rf ~/.config/episodic ~/.config/episodic
rm -rf /tmp/episodic
# 永続データは自己責任で
# rm -rf <memories_dir>
```
...
| 症状 | 確認先 |
|---|---|
| Raw が生成されない | `/tmp/episodic/session-runner.log`、`codex` コマンド存在 |
| Codex の usage limit / API エラーで Raw 失敗 | `~/.local/share/episodic/state/session-retry-queue.jsonl` に自動記録され、次回 SessionStart で `retry-pending.sh` が再生成を試みる。手動再試行は `${CLAUDE_PLUGIN_ROOT}/scripts/session/retry-pending.sh` |
...
| hook が呼ばれない | `/tmp/episodic/session-hook.log`、`/plugin status episodic` |
...
| `--scope web` / `--scope minutes` で常に空 | cocoindex update が一度も成功していない可能性。`/tmp/episodic/cocoindex-update.log` を確認 |
...
# 3. ログで成功確認
tail -f /tmp/episodic/cocoindex-update.log
```
