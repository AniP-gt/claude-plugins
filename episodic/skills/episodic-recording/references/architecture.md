...
16. 後処理:
    ...
    - **staged**（fallback_dir 保存）: 後処理を抑止。次回 SessionStart hook で `scripts/sync-pending.sh` が走り、canary 検出後に staging→`raw/session/` 配下へ移送し、enqueue → wiki-runner（→ cocoindex）も後追いで起動
