...
`episodic` プラグイン（`hooks/hooks.json`）の `Stop` フックが `${CLAUDE_PLUGIN_ROOT}/scripts/session-stop.sh` を起動する。`session-stop.sh` は同梱の `scripts/session/hook.py` を exec する薄いブートストラップで、stdin の JSON ペイロードをそのまま Python hook へパススルーする。

Stop hook は **Claude Code の応答ごと** に発火するため、本フックは debounce タイマーを噛ませて「最後の Stop から `stop_debounce_seconds`（既定 15 秒）静寂が続いたら 1 度だけ Codex を起動する」設計をとる。Claude Code を強制終了したり wrapper 経由で終了させた場合でも、最後に走った Stop の transcript をもとに Codex 要約が走る。
...
8. `/tmp/<session_id>/<ts>.runner.command` ランチャーを生成（実行はまだしない）
...
10. debounce 満了で `--finalize` モードが起動し、`/tmp/<session_id>/` 内の最新 `*.runner.command` を選んで処理中ロック（`/tmp/<session_id>/.lock/`）を取得し、macOS では `open -g -a Terminal` で Terminal.app に渡す。`osascript` / `open` が見つからない環境では Terminal を開かず、launcher を直接バックグラウンドで起動する
11. mac 経路のランチャーは `scripts/session/runner.sh <combined_md> <report_path> <staged|normal> <meta_json>` を実行後、`osascript` で自ウィンドウを `tty` 特定して自動クローズ
12. runner.sh は起動直後に `/tmp/<session_id>/` の最新 timestamp を再選択する（launcher 起動の遅延中に新 Stop が来た場合の救済）
13. Codex (`gpt-5.4-mini`) が同梱 Markdown を解析し、`kind: session` フロントマター付きレポートを上記 `report_path` に直接書き出す
14. runner.sh の `trap EXIT` で `/tmp/<session_id>/` をディレクトリごと削除する。ただし処理に使った `<ts>` より新しい `*.codex.md` が見つかった場合は `--finalize` を再 spawn し、SESSION_DIR は新 finalize の trap EXIT に掃除を委譲する
15. 後処理:
    - **normal**（マウント成立）: `scripts/wiki/enqueue.py` で wiki キューへ追加（kind 自動推定） → `scripts/wiki/wiki-runner.sh` 起動。wiki-runner が処理完了時に **`scripts/lib/cocoindex_trigger.sh` 経由で cocoindex update を 1 回だけキック**する統一経路（runner.sh からの直接トリガーは廃止）
    - **staged**（fallback_dir 保存）: 後処理を抑止。次回 SessionStart hook で `scripts/session/sync-pending.sh` が走り、canary 検出後に staging→`raw/session/` 配下へ移送し、enqueue → wiki-runner（→ cocoindex）も後追いで起動
16. 完了時に macOS 通知で結果を知らせる（`osascript` 不在時はログ追記のみ。ログ集約先は `/tmp/memories/`）
...
両者ともマウント前提（staging なし）。保存後に **`scripts/wiki/enqueue.py --kind <kind>` で wiki キューへ追加 → `scripts/wiki/wiki-runner.sh` を fire-and-forget 起動** する。Codex が kind 別に統合先（`wiki/references.md` / `wiki/minutes/<YYYYMM>.md`）を更新し、wiki-runner 完了時に cocoindex update が 1 回キックされる（統一経路）。

- **web**: `scripts/recording/web/fetch-jina.sh <URL> [--title T] [--tags ...]` が `r.jina.ai/<URL>` から Markdown を取得し、`<MEMORIES_DIR>/raw/web/YYYY-MM-DD/HHMMSS_<slug>.md` に書き出す。`JINA_API_KEY`（環境変数または `~/.config/jina/secrets.env`）があれば Bearer 付与。保存直後に enqueue + wiki-runner 起動
- **minutes**: `scripts/recording/minutes/save.sh --title T [--from-file PATH | stdin]` が本文を frontmatter 付きで `<MEMORIES_DIR>/raw/minutes/YYYY-MM-DD/HHMMSS_<slug>.md` に書き出す。要約・整形は行わない。保存直後に enqueue + wiki-runner 起動
...
- `${CLAUDE_PLUGIN_ROOT}/scripts/session/sync-pending.sh` — staging→正規パス移送（SessionStart hook で起動）
- `${CLAUDE_PLUGIN_ROOT}/scripts/session/session-extract.py` — JSONL部分抽出CLI（再調査用）
- `${CLAUDE_PLUGIN_ROOT}/scripts/recording/main_memory.py` — memory 専用 cocoindex エントリポイント（episodic プラグイン自身の venv（`episodic/scripts/.venv`）で実行）
...
- `${CLAUDE_PLUGIN_ROOT}/scripts/templates/memory.env.example` — `~/.config/memory/.env` のひな形（`MEMORY_DATABASE_URL`）
- `${CLAUDE_PLUGIN_ROOT}/scripts/templates/memory_secrets.env.example` — `~/.config/memory/secrets.env` のひな形（`VOYAGE_API_KEY`）
...
- **一時Markdown（揮発）**: `/tmp/<session_id>/<ts>.md` / `/tmp/<session_id>/<ts>.codex.md` / `/tmp/<session_id>/<ts>.codex.meta.json` / `/tmp/<session_id>/<ts>.runner.command`（runner.sh の trap EXIT がディレクトリごと削除する）
- **hookログ**: `/tmp/memories/session-hook.log`
- **runnerログ**: `/tmp/memories/session-runner.log`
- **syncログ**: `/tmp/memories/session-sync.log`
- **retryログ**: `/tmp/memories/session-retry.log`
...
- レポートが生成されない → `/tmp/memories/session-runner.log` で codex の失敗原因を確認
- hookが呼ばれていない → `/tmp/memories/session-hook.log` を確認、プラグインの `hooks/hooks.json` が有効化されているか確認（`/plugin enable episodic@hidetsugu-miya`）
...
- 取り残し再 finalize の確認 → `/tmp/memories/session-runner.log` の `respawn finalize for newer timestamp:` を grep
...
`runner.sh` の `trigger_cocoindex_update()` が Raw 生成成功時に memory 専用エントリポイント `scripts/recording/main_memory.py` を経由して cocoindex を更新する（best effort・非同期）。

episodic プラグインは **専用 venv（`episodic/scripts/.venv`）と専用 database（`memory`）** を持ち、cocoindex プラグインから完全に独立している。`uv` がインストールされていない環境では cocoindex 更新は自動的にスキップされる。

- **インデックス名**: テーブル名は `memoryindex_<host>_<basename(MEMORIES_DIR)>__chunks` 形式（`<host>` は `socket.gethostname()` を `[^a-zA-Z0-9]` を `_` に置換した小文字。例: `MEMORIES_DIR=/Volumes/memory` の場合 `memoryindex_<host>_memory__chunks`）。`runner.sh` は `INDEX_NAME` を渡し、`scripts/search/search.py` の `get_table_name()` 算出と一致させる
- **App 名**: `MemoryIndex_<host>_<index_name>`
- **データベース**: `MEMORY_DATABASE_URL`（既定 `postgres://postgres:postgres@localhost:15432/memory`）。cocoindex プラグインの `cocoindex` database とは別空間
...
- **モデル変更時**: 次元・ベクトル空間が変わるため、`memory` database 内のテーブル + メタデータを drop してから全件 re-embed が必要

### ハイブリッド検索のためのスキーマ拡張

`main_memory.py` の `_app_main` は HNSW 以外に以下を `declare_sql_command_attachment` で追加する:
...
episodic プラグインの `scripts/` は `uv` の独立 venv（`scripts/.venv`）と独立 database（`memory`）を持つため、cocoindex プラグインのバージョン変更や再インストールに対して頑健。

```bash
# memory テーブルを drop して再構築する手順（cocoindex_setup_metadata は memory DB 内にあるため一括 drop で消える）
HOSTPREFIX="memoryindex_$(hostname | sed 's/[^a-zA-Z0-9]/_/g' | tr A-Z a-z)_memory"
docker exec cocoindex psql -U postgres -d memory -c "
  DROP TABLE IF EXISTS ${HOSTPREFIX}__chunks CASCADE;
  DELETE FROM cocoindex_setup_metadata WHERE flow_name ILIKE '%memory%';
"
