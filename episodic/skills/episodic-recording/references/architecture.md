...
`episodic` プラグイン（`hooks/hooks.json`）の `SessionEnd` フックが `${CLAUDE_PLUGIN_ROOT}/hooks/session-end.sh` を起動する。`session-end.sh` は同梱の `scripts/recording/hook.py` を `exec` するだけの薄いブートストラップで、以下が走る:
