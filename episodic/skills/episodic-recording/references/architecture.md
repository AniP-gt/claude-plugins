...
`episodic` プラグイン（`hooks/hooks.json`）の `SessionEnd` フックが `${CLAUDE_PLUGIN_ROOT}/scripts/session-end.sh` を起動する。`session-end.sh` は呼び出し元を判定して同梱の `scripts/recording/hook.py` に渡す薄いブートストラップで、通常の Claude Code hook では `--source claude`、Codex wrapper 終了時（`CODEX_EXIT_STATUS` が存在する場合）は `--source codex` を指定する。

Claude Code 経路では以下が走る:

1. stdin の JSON (`session_id` / `cwd` / `transcript_path`) を取得
2. 対応する Claude Code JSONL 履歴を特定
...
9. 後処理:
   ...
   - **staged**（fallback_dir 保存）: 後処理を抑止。次回 SessionStart hook で `scripts/recording/sync-pending.sh` が走り、canary 検出後に staging→`raw/session/` 配下へ移送し、enqueue → wiki-runner（→ cocoindex）も後追いで起動
10. 完了時に macOS 通知で結果を知らせる（`osascript` 不在時はログ追記のみ。ログ集約先は `/tmp/memories/`）

### Codex wrapper 経路

Codex wrapper から `SessionEnd` 相当で `session-end.sh` が呼ばれ、`CODEX_EXIT_STATUS` が設定されている場合は `hook.py --source codex` として起動する。この経路は Claude Code の stdin hook payload を使わず、`scripts/recording/codex_source.py` が payload を合成する。

1. `CODEX_WRAPPER_CWD`（なければ `PWD` / `os.getcwd()`）と `CODEX_WRAPPER_START_EPOCH` を読み取る
2. `~/.codex/sessions/**/*.jsonl` から開始時刻以降に更新された候補を新しい順に走査する
3. JSONL 内の `session_meta.payload.cwd` が一致する最新候補を採用し、一致がない場合は最新候補を fallback とする
4. `session_meta.payload.id` / `cwd` / JSONL パス / `CODEX_EXIT_STATUS` から `hook.py` 互換 payload を合成する
5. `scripts/recording/codex-jsonl-to-markdown.py` で Codex CLI の `session_meta` / `response_item` 形式を Markdown 化する
6. 以降は Claude Code 経路と同じ `run()` に合流し、保存先解決、Terminal launcher、Codex 要約、wiki enqueue、cocoindex update まで同じ後処理を使う

Codex JSONL の metadata scan は `codex_source.scan_metadata()` が担当する。`git_branch` は Codex JSONL から取得できないため `unknown` とし、`model` は `session_meta.payload.model` の最多値を使う。
...
### `project` フィールド生成（`scripts/recording/hook.py`）
...
- `${CLAUDE_PLUGIN_ROOT}/scripts/recording/codex-jsonl-to-markdown.py` — Codex CLI JSONL→Markdown変換
- `${CLAUDE_PLUGIN_ROOT}/scripts/recording/codex_source.py` — Codex wrapper 用 payload 合成・metadata scan
...
- hookが呼ばれていない → `/tmp/memories/recording-hook.log` を確認、プラグインの `hooks/hooks.json` が有効化されているか確認（`/plugin enable episodic@hidetsugu-miya`）
- SKIP判定された → 会話にユーザープロンプトがない、または作業実体がないと判定されている
- Codex wrapper 経路で JSONL が見つからない → `/tmp/memories/recording-hook.log` の `codex sessions dir not found` / `Codex JSONL not found` を確認し、`~/.codex/sessions/`、`CODEX_WRAPPER_CWD`、`CODEX_WRAPPER_START_EPOCH` の値を確認
