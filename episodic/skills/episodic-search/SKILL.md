...
description: エピソード記憶（memories/raw/{session,web,minutes} + memories/wiki + diary_dir/raw/diary）に対する全文ベクトル検索 skill。cocoindex バックエンドでセマンティック検索し、scope（session/web/minutes/wiki/diary/all）と status（active のみ / superseded 含む）でフィルタする。Claude Code 内からも、Claude API 経由で外部アプリからも利用できる。「memoriesから○○を検索して」「過去のセッションで○○を扱ったものを探して」「web だけで○○を検索」「日記から○○を探して」等で起動する。
argument-hint: <query> [--top N] [--scope session|web|minutes|wiki|diary|all] [--include-superseded] [--format json|markdown] [--no-dedupe] [--low-score-threshold N]
context: fork
effort: low
---
...
`memories/` 配下（Raw（kind: session / web / minutes）+ Wiki）および `diary_dir/` 配下（Raw（kind: diary）、ローカル限定）に対する全文ベクトル検索を提供する薄い skill。cocoindex プラグインを内部で呼び出し、結果に scope/status フィルタを適用して返す。cocoindex は memories_dir と diary_dir の 2 ソースを 1 テーブルに走査する。
...
| `--scope session\|web\|minutes\|wiki\|diary\|all` | | all | 検索対象を絞る（diary はローカル限定。`all` にも含まれる） |
...
- `MEMORIES_DIARY_DIR`: kind: diary 専用ローカルルートの絶対パス（既定: `~/.local/share/episodic/diary`）。`config.toml` の `diary_dir` でも設定可。`search.sh` が `--diary-dir` として `format.py` に渡し、diary パスの絶対化と scope 判定に使う
...
# kind: diary だけで絞る（プライベート日記、ローカル限定）
...
`recent.sh` はベクトル検索を使わず、`raw/<kind>/` 配下の日付ディレクトリ＋ファイル名タイムスタンプで時系列ソートする補助スクリプト。「直近の作業を見せて」「今日のセッション一覧」「最近アーカイブした URL」のような、意味検索ではなく時系列で取り出したい場面で使う。`--kind` 既定は `session`、`web` / `minutes` / `diary` / `all` も指定可能。`--project` で絞り込み（kind=session で意味あり）、`--days` で期間制限、`--format paths` でパスのみ抽出も可能。`diary` はローカル限定かつプライバシー既定として `--kind all` には含まれず、明示的に `--kind diary` を指定したときのみ一覧される。
...
- `--scope diary` で常に空 → diary の記録がまだない、または `DIARY_SOURCE_PATH` を渡さずに `cocoindex update` した。`episodic-recording` skill の diary セクションから保存し、`cocoindex_trigger.sh` 経由で再インデックスする
